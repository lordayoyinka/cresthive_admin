...
const CmsSecondary = () => {
  const [secondaryPageData, setSecondaryPageData] = useState(null);
  const [isEditing, setEditing] = useState(false);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, 'cms', 'secondaryPage');
        const docSnapshot = await getDoc(docRef);

        if (docSnapshot.exists) {
          setSecondaryPageData(docSnapshot.data());
  ...
  const handleSaveChanges = async () => {
    try {
      const docRef = doc(db, 'cms', 'secondaryPage');
      await setDoc(docRef, secondaryPageData);
      setEditing(false);
    ...
    useEffect(() => {
        ...
        const resave = async () => {
          try {
            const docRef = doc(db, "cms", "secondaryPage");
            await setDoc(docRef, mockSecondaryPageData);
      ...
      return (
        <div className="container mx-auto px-6 overflow-y-auto h-full pb-40 my-8">
          <h1 className="text-4xl font-bold mb-4">CMS Secondary Page</h1>
          {secondaryPageData && (
            <div className="max-w-2xl mx-auto">
              <h2 className="text-3xl mb-2">
                {isEditing ? (
                  <textarea
                    ...
                    type="text"
                    value={secondaryPageData.heroTitle}
                    onChange={(e) => handleInputChange('heroTitle', e.target.value)}
                  />
                ) : (
                  secondaryPageData.heroTitle
                )}
              </h2>
              <p className="mb-4">
                {isEditing ? (
                  <textarea
                    ...
                    type="text"
                    rows={4}
                    value={secondaryPageData.heroSubtitle}
                    onChange={(e) => handleInputChange('heroSubtitle', e.target.value)}
                  />
                ) : (
                  secondaryPageData.heroSubtitle
                )}
              </p>
              ...
              <ul>
                {secondaryPageData.notables.map((notable, index) => (
